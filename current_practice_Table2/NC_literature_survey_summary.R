...
references <- read_csv("NC_literature_survey.csv") %>% clean_names() %>%
  ...
  select(journal, papers, is_falsification, contains("falsification"), contains("test_type"), iv_control_in_pi_test, nc_count) %>%
...
write_csv(summary_table_final, file = "survey_summary.csv")

# --- Compute inline LaTeX stats ------------------------------

# delete stats.tex if it exists:
if (file.exists("stats.tex")) {
...
write_stat <- function(stat, commandName) {
  ...
  cat(sprintf("\\newcommand{\\%s}{%.0f\\%%}\n", commandName, stat*100), file = "stats.tex", append = TRUE)
}
